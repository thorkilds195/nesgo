...
func setupTestBus(program []uint8) *Bus {
	test_header := []uint8{0x4e, 0x45, 0x53, 0x1a, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
	mem := make([]uint8, 100000)
	copy(mem, test_header)
	copy(mem[len(test_header):], program)
	// Make sure the reset vector exists, these tests start execution at 0x8000
	// so since prg rom adress 0x0000 is 0x8000, make the necesarry write adjustment
	// also accounting for the 16 bit rom header
	mem[0xFFFC-0x8000+16] = 0x00
	mem[0xFFFD-0x8000+16] = 0x80
	// Same operation but for interrupt vector
...
func TestLDAAbsolute(t *testing.T) {
	vec := []uint8{0xAD, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1005, 10)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 10)
	assert_status(t, c.status, 0b0000_0100)
...
func TestLDAAbsoluteX(t *testing.T) {
	vec := []uint8{0xa2, 0x92, 0xBD, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 10)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 10)
	assert_status(t, c.status, 0b0000_0100)
...
func TestLDAAbsoluteY(t *testing.T) {
	vec := []uint8{0xA0, 0x92, 0xB9, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 10)
	c.Reset()
	c.Run()
...
func TestLDAIndirectX(t *testing.T) {
	...
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x24, 0x15)
	c.MemWrite(0x25, 0x10)
	c.MemWrite16(0x1015, 10)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 10)
	assert_status(t, c.status, 0b0000_0100)
...
func TestLDAIndirectY(t *testing.T) {
	...
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x20, 0x10)
	c.MemWrite(0x21, 0x10)
	c.MemWrite16(0x1014, 10)
	c.Reset()
	c.Run()
	c.Reset()
	c.Run()
...
func TestLDXAbsoluteY(t *testing.T) {
	vec := []uint8{0xA0, 0x92, 0xBE, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 10)
	c.Reset()
	c.Run()
...
func TestLDYAbsoluteX(t *testing.T) {
	vec := []uint8{0xa2, 0x92, 0xBC, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 10)
	c.Reset()
	c.Run()
...
func TestInxOverflowTo0(t *testing.T) {
	vec := []uint8{0xa9, 0xff, 0xAA, 0xe8, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.Reset()
	c.Run()
...
func TestInxOverflow(t *testing.T) {
	vec := []uint8{0xa9, 0xff, 0xAA, 0xe8, 0xe8, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.Reset()
	c.Run()
	assert_register(t, c.register_x, 1)
...
func TestInxWhenBit7Set(t *testing.T) {
	vec := []uint8{0xa9, 200, 0xAA, 0xe8, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.Reset()
	c.Run()
...
func TestInyOverflowTo0(t *testing.T) {
	vec := []uint8{0xa0, 0xff, 0xAA, 0xC8, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.Reset()
	c.Run()
...
func TestInyOverflow(t *testing.T) {
	vec := []uint8{0xa0, 0xff, 0xAA, 0xC8, 0xC8, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.Reset()
	c.Run()
	assert_register(t, c.register_y, 1)
...
func TestInyWhenBit7Set(t *testing.T) {
	vec := []uint8{0xa0, 200, 0xAA, 0xC8, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.Reset()
	c.Run()
...
func TestAdcAbsolute(t *testing.T) {
	...
	// Runs adc instr with 0x15 and 0x10
	// This should fetch from memory location 0x8010
	// and add the current value of a register to it (0x01)
	vec := []uint8{0xa9, 0x01, 0x6D, 0x15, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1015, 10)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 11)
}
...
func TestAdcAbsoluteX(t *testing.T) {
	...
	// and add the current value of a register to it (0x01)
	vec := []uint8{0xa9, 0x01, 0xa2, 0x92, 0x7D, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 10)
	c.Reset()
	c.Run()
...
func TestAdcAbsoluteY(t *testing.T) {
	...
	// This should fetch from memory location 0x2092
	// and add the current value of a register to it (0x01)
	vec := []uint8{0xa9, 0x01, 0xa0, 0x92, 0x79, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 10)
	c.Reset()
	c.Run()
...
func TestAdcIndirectX(t *testing.T) {
	...
	c.MemWrite(0x25, 0x15)
	c.MemWrite16(0x1510, 10)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 11)
	assert_status(t, c.status, 0b0000_0100)
...
func TestAdcIndirectY(t *testing.T) {
	...
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x20, 0x10)
	c.MemWrite(0x21, 0x15)
	c.MemWrite16(0x1514, 10)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 11)
	assert_status(t, c.status, 0b0000_0100)
...
func TestANDAbsolute(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0011, 0x2D, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1005, 0b1000_0001)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 0b0000_0001)
	assert_status(t, c.status, 0b0000_0100)
...
func TestANDAbsoluteX(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0011, 0xa2, 0x92, 0x3D, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 0b1000_0001)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 0b0000_0001)
	assert_status(t, c.status, 0b0000_0100)
...
func TestANDAbsoluteY(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0011, 0xA0, 0x92, 0x39, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 0b1000_0001)
	c.Reset()
	c.Run()
...
func TestANDIndirectX(t *testing.T) {
	...
	c.MemWrite(0x24, 0x10)
	c.MemWrite(0x25, 0x15)
	c.MemWrite16(0x1510, 0b1000_0001)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 0b0000_0001)
	assert_status(t, c.status, 0b0000_0100)
...
func TestANDIndirectY(t *testing.T) {
	...
	c.MemWrite(0x20, 0x10)
	c.MemWrite(0x21, 0x15)
	c.MemWrite16(0x1514, 0b1000_0001)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 0b0000_0001)
	assert_status(t, c.status, 0b0000_0100)
...
func TestASLAbsolute(t *testing.T) {
	vec := []uint8{0x0E, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1005, 0b0000_0011)
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1005), 0b0000_0110)
	assert_status(t, c.status, 0b0000_0100)
}

func TestASLAbsoluteX(t *testing.T) {
	vec := []uint8{0xA2, 0x02, 0x1E, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1007, 0b0000_0011)
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1007), 0b0000_0110)
	assert_status(t, c.status, 0b0000_0100)
}
...
func TestBITAbsolute(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0010, 0x2C, 0x10, 0x15, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1510, 0b0000_0010)
	c.Reset()
	c.Run()
	assert_status(t, c.status, 0b0000_0100)
}
...
func TestCMPImmediateWhen7BitSet(t *testing.T) {
	...
	c.Reset()
	c.Run()
	assert_status(t, c.status, 0b0010_0111)
}
...
func TestCMPAbsoluteWhenAGreaterThanM(t *testing.T) {
	vec := []uint8{0xA9, 0x09, 0xCD, 0x50, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1050, 0x05)
	c.Reset()
	c.Run()
	assert_status(t, c.status, 0b0000_0101)
}
...
func TestCMPAbsoluteWhen7BitSet(t *testing.T) {
	vec := []uint8{0xA9, 0xFF, 0xCD, 0x50, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1050, 0xFF)
	c.Reset()
	c.Run()
	assert_status(t, c.status, 0b0010_0111)
}
...
func TestCMPAbsoluteXWhenAGreaterThanM(t *testing.T) {
	vec := []uint8{0xa2, 0x01, 0xA9, 0x09, 0xDD, 0x50, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1051, 0x05)
	c.Reset()
	c.Run()
	assert_status(t, c.status, 0b0000_0101)
}
...
func TestCMPAbsoluteXWhen7BitSet(t *testing.T) {
	vec := []uint8{0xa2, 0x01, 0xA9, 0xFF, 0xDD, 0x50, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1051, 0xFF)
	c.Reset()
	c.Run()
	assert_status(t, c.status, 0b0010_0111)
}
...
func TestCMPAbsoluteYWhenAGreaterThanM(t *testing.T) {
	vec := []uint8{0xA0, 0x01, 0xA9, 0x09, 0xD9, 0x50, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1051, 0x05)
	c.Reset()
	c.Run()
...
func TestCMPAbsoluteYWhen7BitSet(t *testing.T) {
	vec := []uint8{0xA0, 0x01, 0xA9, 0xFF, 0xD9, 0x50, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1051, 0xFF)
	c.Reset()
	c.Run()
...
func TestCPXAbsoluteWhenAGreaterThanM(t *testing.T) {
	vec := []uint8{0xA2, 0x09, 0xEC, 0x50, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1050, 0x05)
	c.Reset()
	c.Run()
...
func TestCPXAbsoluteWhen7BitSet(t *testing.T) {
	vec := []uint8{0xA2, 0xFF, 0xEC, 0x50, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1050, 0xFF)
	c.Reset()
	c.Run()
...
func TestCPYAbsoluteWhenAGreaterThanM(t *testing.T) {
	vec := []uint8{0xA0, 0x09, 0xCC, 0x50, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1050, 0x05)
	c.Reset()
	c.Run()
...
func TestCPYAbsoluteWhen7BitSet(t *testing.T) {
	vec := []uint8{0xA0, 0xFF, 0xCC, 0x50, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1050, 0xFF)
	c.Reset()
	c.Run()
...
func TestDECZeroAbsolute(t *testing.T) {
	vec := []uint8{0xCE, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1005, 0x02)
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1005), 0x01)
	assert_status(t, c.status, 0b0000_0100)
}

func TestDECZeroAbsoluteX(t *testing.T) {
	vec := []uint8{0xA2, 0x01, 0xDE, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1006, 0x02)
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1006), 0x01)
	assert_status(t, c.status, 0b0000_0100)
}
...
func TestEORAbsolute(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0101, 0x4D, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1005, 0b0000_0011)
	c.Reset()
	c.Run()
...
func TestEORAbsoluteX(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0101, 0xa2, 0x92, 0x5D, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 0b0000_0011)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 0b0000_0110)
	assert_status(t, c.status, 0b0000_0100)
...
func TestEORAbsoluteY(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0101, 0xA0, 0x92, 0x59, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 0b0000_0011)
	c.Reset()
	c.Run()
...
func TestEORIndirectX(t *testing.T) {
	...
	c.MemWrite(0x24, 0x10)
	c.MemWrite(0x25, 0x15)
	c.MemWrite16(0x1510, 0b0000_0011)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 0b0000_0110)
	assert_status(t, c.status, 0b0000_0100)
...
func TestEORIndirectY(t *testing.T) {
	...
	c.MemWrite(0x20, 0x10)
	c.MemWrite(0x21, 0x15)
	c.MemWrite16(0x1514, 0b0000_0011)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 0b0000_0110)
	assert_status(t, c.status, 0b0000_0100)
...
func TestINCAbsolute(t *testing.T) {
	vec := []uint8{0xEE, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1005, 0x02)
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1005), 0x03)
	assert_status(t, c.status, 0b0000_0100)
}

func TestINCAbsoluteX(t *testing.T) {
	vec := []uint8{0xA2, 0x01, 0xFE, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1006, 0x02)
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1006), 0x03)
	assert_status(t, c.status, 0b0000_0100)
}

// JMP
func TestJMPAbsolute(t *testing.T) {
	vec := []uint8{0x4C, 0x01, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1001, 0xA2)
	c.MemWrite(0x1002, 0x09)
	c.Reset()
	c.Run()
...
func TestJMPIndirect(t *testing.T) {
	vec := []uint8{0x6C, 0x01, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite16(0x1001, 0x1010)
	c.MemWrite(0x1010, 0xA2)
	c.MemWrite(0x1011, 0x09)
	c.Reset()
	c.Run()
	assert_register(t, c.register_x, 0x09)
	assert_status(t, c.status, 0b0000_0100)
...
func TestLSRAbsolute(t *testing.T) {
	vec := []uint8{0x4E, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1005, 0b0000_0110)
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1005), 0b0000_0011)
	assert_status(t, c.status, 0b0000_0100)
}

func TestLSRAbsoluteX(t *testing.T) {
	vec := []uint8{0xA2, 0x02, 0x5E, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1007, 0b0000_0110)
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1007), 0b0000_0011)
	assert_status(t, c.status, 0b0000_0100)
}
...
func TestORAAbsolute(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0101, 0x0D, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1005, 0b0000_0011)
	c.Reset()
	c.Run()
...
func TestORAAbsoluteX(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0101, 0xa2, 0x92, 0x1D, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 0b0000_0011)
	c.Reset()
	c.Run()
...
func TestORAAbsoluteY(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0101, 0xA0, 0x92, 0x19, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 0b0000_0011)
	c.Reset()
	c.Run()
...
func TestROLAbsolute(t *testing.T) {
	vec := []uint8{0x2E, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1005, 0b0000_0110)
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1005), 0b0000_1100)
	assert_status(t, c.status, 0b0000_0100)
}

func TestROLAbsoluteX(t *testing.T) {
	vec := []uint8{0xA2, 0x02, 0x3E, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1007, 0b0000_0110)
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1007), 0b0000_1100)
	assert_status(t, c.status, 0b0000_0100)
}
...
func TestRORAbsoluteX(t *testing.T) {
	vec := []uint8{0xA2, 0x02, 0x7E, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1007, 0b0000_0110)
	c.Reset()
	c.Run()
...
func TestRORAccumulatorWhenCarrySet(t *testing.T) {
	...
	c.status = 0b0000_0001
	c.Run()
	assert_register(t, c.register_a, 0b1000_0110)
	assert_status(t, c.status, 0b1000_0100)
}
...
func TestRORZeroPageWhenCarrySet(t *testing.T) {
	...
	c.status = 0b0000_0001
	c.Run()
	assert_register(t, c.MemRead(0xF8), 0b1000_0110)
	assert_status(t, c.status, 0b1000_0100)
}
...
func TestSBCAbsolute(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0101, 0xED, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1005, 0b0000_0001)
	c.Reset()
	c.status = 0b0000_0001
	c.Run()
	assert_register(t, c.register_a, 0b0000_0100)
...
func TestSBCAbsoluteX(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0101, 0xa2, 0x92, 0xFD, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 0b0000_0001)
	c.Reset()
	c.status = 0b0000_0001
	c.Run()
	assert_register(t, c.register_a, 0b0000_0100)
...
func TestSBCAbsoluteY(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0101, 0xA0, 0x92, 0xF9, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1092, 0b0000_0001)
	c.Reset()
	c.status = 0b0000_0001
...
func TestSBCIndirectX(t *testing.T) {
	...
	c.MemWrite(0x24, 0x10)
	c.MemWrite(0x25, 0x15)
	c.MemWrite16(0x1510, 0b0000_0001)
	c.Reset()
	c.status = 0b0000_0001
	c.Run()
	assert_register(t, c.register_a, 0b0000_0100)
...
func TestSBCIndirectY(t *testing.T) {
	...
	c.MemWrite(0x20, 0x10)
	c.MemWrite(0x21, 0x15)
	c.MemWrite16(0x1514, 0b0000_0001)
	c.Reset()
	c.status = 0b0000_0001
	c.Run()
	assert_register(t, c.register_a, 0b0000_0100)
...
func TestSTAAbsolute(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0101, 0x8D, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1005), c.register_a)
	assert_status(t, c.status, 0b0000_0100)
}

func TestSTAAbsoluteX(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0101, 0xa2, 0x02, 0x9D, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1002), c.register_a)
	assert_status(t, c.status, 0b0000_0100)
}

func TestSTAAbsoluteY(t *testing.T) {
	vec := []uint8{0xA9, 0b0000_0101, 0xA0, 0x02, 0x99, 0x00, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1002), c.register_a)
...
func TestSTAIndirectX(t *testing.T) {
	...
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1510), c.register_a)
	assert_status(t, c.status, 0b0000_0100)
}
...
func TestSTAIndirectY(t *testing.T) {
	...
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1514), c.register_a)
	assert_status(t, c.status, 0b0000_0100)
}
...
func TestSTXAbsolute(t *testing.T) {
	vec := []uint8{0xA2, 0b0000_0101, 0x8E, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1005), 0b0000_0101)
	assert_status(t, c.status, 0b0000_0100)
}
...
func TestSTYAbsolute(t *testing.T) {
	vec := []uint8{0xA0, 0b0000_0101, 0x8C, 0x05, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.Reset()
	c.Run()
	assert_register(t, c.MemRead(0x1005), c.register_y)
	assert_status(t, c.status, 0b0000_0100)
}
...
func TestJSRStackDecrement(t *testing.T) {
	...
	if !(c.MemRead16(0x0100+uint16(STACK_RESET-1)) == 0x8000+2) {
		t.Error("Stack pointer return value is wrong")
	}
}

func TestJSRLDA(t *testing.T) {
	vec := []uint8{0x20, 0x01, 0x10, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1001, 0xA9)
	c.MemWrite(0x1002, 0x09)
	c.Reset()
	c.Run()
...
func TestRTS(t *testing.T) {
	vec := []uint8{0x20, 0x01, 0x10, 0xA2, 0x09, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.MemWrite(0x1001, 0xA9)
	c.MemWrite(0x1002, 0x12)
	c.MemWrite(0x1003, 0x60)
	c.Reset()
	c.Run()
	assert_register(t, c.register_a, 0x12)
	assert_register(t, c.register_x, 0x09)
...
func TestFiveOpsWorkingTogether(t *testing.T) {
	vec := []uint8{0xa9, 0xc0, 0xaa, 0xe8, 0x00}
	c := InitCPU(setupTestBus(vec))
	c.Reset()
	c.Run()
